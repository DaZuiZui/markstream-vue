...
2.0 首帧阶段的上升来自 markstream-core 1.1 新增的**流式代码围栏原子提交**：reveal 在未闭合的 ``` 处暂停，围栏（marker、info 行、内容）作为一个原子单元提交。包含多个围栏的大初始文档因此被切成更多、更小的围栏对齐提交（本次运行 parse tail 从 1 增至 12），每次提交多一次解析与布局，直到围栏闭合。这是流式正确性特性（1.x 会把半开的围栏提前暴露给渲染器），并非代码块表面的成本——且只影响初始 pacing 阶段：交互滚动与流式回放严格更优。引用任何单个数字前请在本机重跑。
