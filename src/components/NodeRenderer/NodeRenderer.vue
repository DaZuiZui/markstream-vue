...
function getVisibleDomHeight(preMeasuredElementHeights?: Map<number, number>) {
  // Prefer the (index → height) capture from the same-pass
  // measureTrackedNodeHeights() scan: reusing those readings skips a second
  ...
  if (preMeasuredElementHeights) {
    let total = 0
    for (const height of preMeasuredElementHeights.values())
      total += height > 0 ? height : 0
...
function getVirtualMetrics(
  ...
  phase?: MarkstreamVirtualPhase,
  preMeasuredElementHeights?: Map<number, number>,
): MarkstreamVirtualMetrics {
  const summary = buildVirtualHeightSummary()
  ...
  return {
    ...
    visibleDomHeight: getVisibleDomHeight(preMeasuredElementHeights),
...
function flushVirtualMetricsEmit() {
  ...
  let measuredElementHeights: Map<number, number> | undefined
  ...
  if (pendingHeightMeasurements.size > PENDING_HEIGHT_FORCE_FLUSH_THRESHOLD || heightMeasurementRaf == null)
    forceFlushPendingHeightMeasurements()
  emitVirtualMetricsNow(getVirtualMetrics(pendingVirtualMetricsReason, undefined, measuredElementHeights))
}
...
function measureNodeHeight(index: number, el: HTMLElement, preMeasuredHeight?: number) {
  const height = preMeasuredHeight ?? getNodeLayoutHeight(index, el)
