...
describe('code block runtime interop', () => {
  it('preloads the optional stream-diffs runtime once', async () => {
    vi.resetModules()
    const streamDiffs = await import('stream-diffs/markstream')
    ;(streamDiffs.preloadStreamDiffs as any).mockClear?.()
    ...
    expect(streamDiffs.preloadStreamDiffs).toHaveBeenCalledTimes(1)
